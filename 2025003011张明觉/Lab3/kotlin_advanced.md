...
fun printFinalTemperature(
    ...
    val finalMeasurement = String.format("%.2f", conversionFormula(initialMeasurement)) // 保留两位小数
    println("$initialMeasurement degrees $initialUnit is $finalMeasurement degrees $finalUnit.")
}
```
...
// // 歌曲类：定义歌曲的结构
class Song(
    // 歌曲属性：歌名、音乐人、发行年份、播放次数
    val title: String,
    val artist: String,
    val releaseYear: Int,
    val playCount: Int
) {
    // 判断歌曲是否流行：播放次数 >= 1000 为流行
...
// 测试代码
fun main() {
    // 创建歌曲实例1：播放次数 2500（流行）
    val song1 = Song("Love Story", "Taylor Swift", 2008, 2500)
    song1.printSongDescription()
    println("Is popular: ${song1.isPopular}")

    println("----------------------------------------")

    // 创建歌曲实例2：播放次数 999（不流行）
    val song2 = Song("Small Song", "Unknown Artist", 2024, 500)
...
class FoldablePhone : Phone() {
    // 属性：标记手机是否处于折叠状态（默认折叠）
    private var isFolded: Boolean = true

    // 重写打开屏幕方法：只有展开时才能点亮屏幕
...
fun main() {
    ...
    // 测试4：再次折叠手机
    foldablePhone.fold()
    foldablePhone.checkPhoneScreenLight() // 保持 on（折叠不影响已亮屏幕）
}
```
